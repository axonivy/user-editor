...
const tsOut = path.resolve('./src/data/user.ts');
var schemaUri =
  'https://jenkins.ivyteam.io/job/core_json-schema/job/master/lastSuccessfulBuild/artifact/workspace/ch.ivyteam.ivy.user.role.lsp.schema/target/schema/user/14.0.0/users-tsgen.json';
...
if (schemaUri.startsWith('http')) {
  loadJson(schemaUri)
    .then(schema => tsGen.compile(schema, 'users'))
    .then(ts => writeSrc(ts));
} else {
